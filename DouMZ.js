// DouMZ.js - 斗母猪 SillyTavern 扩展（完整版）
(function () {
    'use strict';

    const EXT_NAME = 'DouSow';
    const STORAGE_KEY = 'dousow_state_v4';
    const EFFECT_KEY = 'dousow_effects_v4';
    const UI_KEY = 'dousow_ui_v4';
    ...
    function addEffect(playerIdx, rank, count, forcePermanent) {
        ...
        if (rank === '7') {
            ...
            if (p7.clothes && p7.clothes.trim()) return;
    ...
    function createUI() {
        ...
        editBtn.textContent = '⚙'; editBtn.id = 'ds-edit';
        btns.appendChild(editBtn);

        var bgBtn = document.createElement('button');
        bgBtn.textContent = '🎨'; bgBtn.id = 'ds-bg';
        btns.appendChild(bgBtn);

        var minBtn = document.createElement('button');
        minBtn.textContent = '—'; minBtn.id = 'ds-min';
    ...
    function renderUI() {
        ...
        for (var i = 0; i < 3; i++) {
            ...
            var clothesRow = document.createElement('div');
            clothesRow.style.fontSize = '10px';
            clothesRow.textContent = '衣物：';
            var clothesInput = document.createElement('input');
            clothesInput.type = 'text';
            clothesInput.value = p.clothes || '';
            clothesInput.setAttribute('data-clothes', i);
            clothesInput.style.width = '70%';
            clothesInput.style.background = '#2a1010';
            clothesInput.style.color = '#fff';
            clothesInput.style.border = '1px solid #666';
            clothesRow.appendChild(clothesInput);
            card.appendChild(clothesRow);
        ...
        var clothesInputs = body.querySelectorAll('[data-clothes]');
        for (var c = 0; c < clothesInputs.length; c++) {
            (function (inp) {
                inp.onchange = function () {
                    var idx = parseInt(inp.getAttribute('data-clothes'));
                    G.players[idx].clothes = inp.value;
                    saveAll(); injectState();
                };
            })(clothesInputs[c]);
        }
    ...
    function openEffectEditor() {
        ...
        btnRow.appendChild(mkBtn('关闭', function () { div.remove(); }));
    ...
    function setupEvents() {
        try {
            ...
            c.eventSource.on(c.eventTypes.MESSAGE_DELETED, function () {
                console.log('[DouSow] 消息被删除，可手动点撤回');
            });
